...
func (h *RegionHandler) GetRegion(c *gin.Context) {
	...
	region, err := h.regionService.GetRegionByID(c.Request.Context(), int32(id))
	if err != nil {
...
func (h *RegionHandler) DeleteRegion(c *gin.Context) {
	...
	if err := h.regionService.DeleteRegion(c.Request.Context(), int32(id)); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"error":   true,
...
func (h *RegionHandler) GetRegionByIdWithCountry(c *gin.Context) {
	...
	region, err := h.regionService.GetRegionByIDWithCountry(c.Request.Context(), int32(id))
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{
			"error":   true,
			"message": "Region not found",
